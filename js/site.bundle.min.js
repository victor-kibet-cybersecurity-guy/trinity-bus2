...
window.TBE={phone:"+254754932823",whatsapp:"https://wa.me/254754932823",money:n=>new Intl.NumberFormat("en-KE",{style:"currency",currency:"KES",maximumFractionDigits:0}).format(n),toast(msg){const el=document.getElementById("toast");if(!el)return;el.textContent=msg;el.classList.add("show");setTimeout(()=>el.classList.remove("show"),2600)},ref(){return `TBE-${new Date().getFullYear()}-${Math.floor(10000+Math.random()*90000)}`}};
...
document.addEventListener("DOMContentLoaded",()=>{
 ...
 if(progress){addEventListener("scroll",onScroll,{passive:true});onScroll()}
 ...
 const q=document.querySelector("[data-faq-search]");if(q)q.addEventListener("input",()=>{const v=q.value.toLowerCase().trim();document.querySelectorAll("[data-help-item]").forEach(x=>x.hidden=!!v&&!x.textContent.toLowerCase().includes(v))});
});
